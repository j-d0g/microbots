# Runtime Pattern → microbots Translation

## TL;DR

The upstream codebase treats every workflow as a self-contained PEP-723 Python script that boots a FastAPI app on a port. A central coordinator (`<internal-runtime>/service.py`) provisions an E2B sandbox per request, injects `process_request.py`, which `uv run`s `server.py`, streams logs, and posts the result back. Schedules are an EventBridge → Lambda → `/run_async` HTTP poke. For microbots v0, the simplest port is: **one Render Web Service for agent+UI, one Render Web Service per promoted microbot (Devin scaffolds, Render auto-deploys from git), Render Cron Jobs for scheduling, no E2B**. E2B is over-engineered for a hackathon.

---

## the upstream codebase workflow architecture as I understand it
...
- **PEP 723 inline metadata** at top: `requires-python`, `dependencies`, and a `[tool.env-checker]` block declaring required env vars (`RUNTIME_API_KEY`, `RUNTIME_URI`, `PORT=8000`, etc.).
...
- Calls to other workflows / library services go through `RUNTIME_URI` (the coordinator), not direct imports — every cross-service call is HTTP.
...
### 2. The coordinator (`<internal-runtime>/service.py`)
...
The coordinator is **the only service that talks to E2B**. It also owns auth (via `platform_auth` forward-auth), Stripe billing, S3 file uploads, and per-service env override resolution (`SecretOverride` → `Secret` → schema default).

### 3. E2B sandbox lifecycle (per-request cold start)

For each `/run_async` call, the coordinator:
1. Creates a fresh E2B sandbox from a pre-built template (`E2B_TEMPLATE_ID=<runtime-template>`). Template bakes in ffmpeg, Python 3.10/3.11/3.12 via `uv`, a warmed `uv` cache, and `/opt/process_request.py`.
2. Uploads the workflow code (pulled from S3 bucket `service-bucket`) to the sandbox.
3. Runs `process_request.py` with env vars: `RUNTIME_API_KEY`, `RUNTIME_URI`, `SERVICE_ID`, `REQUEST_ID`, `PORT`, `REST_OF_PATH`, `REQUEST_HEADERS_JSON`, `REQUEST_BODY_PATH`.
...
### 6. Client SDK (`runtime-client`)

`pip install runtime-client`. The SDK (`runtime_client.py`) wraps the coordinator: `provision_request()`, `run_service(service_id, inputs, in_background=True)`, `start_service()`, `AsyncRuntimeResponse.logs()` / `.result(timeout)` / `.details()`. Auto-injects `X-Correlation-Id` from structlog contextvars. Inside a workflow, `run_service` is how you call other workflows — the SDK is a thin httpx client over the public coordinator endpoints. There's also a `redis_coordinator` module for cross-step coordination (Redis pub/sub for fan-out / wait-for-event patterns), but most workflows don't need it.
...
| the upstream codebase primitive | Render equivalent | Notes |
|---|---|---|
...
| S3 (`service-bucket` for code, `uploads-bucket` for files) | Git (for code, since Render builds from git) + Render Disk or S3 for files | Code-in-S3 was needed because E2B sandboxes needed to download it; once we deploy from git, this disappears. |
...
4. **E2B**: keep ONLY for the unpromoted exploratory phase. When the agent is testing a candidate automation in conversation (before promotion), run it in E2B. Once promoted, code goes to git → Render → trusted execution. This matches the upstream codebase's "always-on URL override" pattern (`<SERVICE_ID>_SERVICE_URL`) but inverts the default: trusted-deployed is the norm, sandbox is the exception.

**What to copy verbatim from the upstream codebase:**
