...
// In M1 plumbing, save_workflow lives on the MCP server, not the frontend.
// Path: <repo>/agent/harness/mcp/saved/  (relative to this test file).
const SAVED_DIR = path.resolve(__dirname, "../../../../harness/mcp/saved");
...
test("v1 save_workflow — agent writes file and returns URL", async ({ page }) => {
  ...
  await expect(saveCall.first()).toBeVisible({ timeout: 90_000 });

  // Wait for the call to resolve (state becomes "result"), then check final text.
  await expect(saveCall.first()).toHaveAttribute("data-tool-state", "result", { timeout: 90_000 });

  const assistantText = page.locator('[data-role="assistant"] [data-testid="message-text"]');
  await expect(assistantText.last()).toContainText(/example\.com\/workflows\/test-ping/i, { timeout: 90_000 });
